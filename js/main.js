...
function buildMedidas() {
  ...
  for (var i = 0; i < tipoMedidas.length; i++) {
    $("#reguladores-fluxo-medidas").append($('<option class="op"> </option>').val(i).html(tipoMedidas[i]));
    $("#sensores-medidas").append($('<option class="op"> </option>').val(i).html(tipoMedidas[i]));
    $("#sistemas-gestao-medidas").append($('<option class="op"> </option>').val(i).html(tipoMedidas[i]));
  }

  //Constroi select de fonte de luz das medidas
  for (i = 0; i < medidas.length; i++) {
    if (medidas[i].tipo === FONTE_LUZ) {
      var add = false;
      for (j = 0; j < fonteLuz.length; j++) {
        if (fonteLuz[j].potenciaUnitFinal == medidas[i].nome) {
          add = true;
          break;
        }
      }
      if (!add) {
        $("#fonte-luz-medidas").append($('<option class="op"> </option>').val(i).html(medidas[i].nome));
      }
    }
...
function recolheDados() {
  ...
  while (espaco.length > 1) { espaco.pop(); }
  for (var i = 1; i <= espacoCount; i++) {
    if (estado == INTERIOR) {
...
function recolheFonteLuz() {
  var elementos = document.getElementsByName("espaco-fonte-luz");
  while (fonteLuz.length > 1) { fonteLuz.pop(); }
  for (var i = 1; i <= espacoCount; i++) {
    ...
    if (tipo == OUTROS)
      ...
      var potencia = $(elementos[i]).find("td").eq(4).find("select").val();
    ...
    if (potenciaUnitFinal >= 25) estadoPotenciaUnitFinal = BP;
    else if (potenciaUnitFinal < 25) estadoPotenciaUnitFinal = AP;
  ...
  /*alert(fonteLuz[1].tipo);
  alert(tipoFonteLuz[fonteLuz[1].tipo].potencia[fonteLuz[1].potencia]);
  alert(JSON.stringify(fonteLuz));*/
  checkCamposFonteLuz();
}
...
function recolheMedidas() {
  var elementos = document.getElementsByName("espaco-medidas");
  while (medidasDados.length > 1) { medidasDados.pop(); }
  for (var i = 1; i <= espacoCount; i++) {
    var designacao = $(elementos[i]).find("td").eq(1).text();
